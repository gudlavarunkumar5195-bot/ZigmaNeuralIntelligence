...
export async function apiListMonitoring(): Promise<ApiResponse<MonitoringConfig[]>> {
...
export async function apiListAlerts(): Promise<ApiResponse<MonitoringAlert[]>> {
...
export async function apiGetMonitoringRules(id: string) {
  return apiFetch<MonitoringRule[]>(
    `/monitoring/${encodeURIComponent(id)}/rules`,
  )
}
...
export async function apiPlanWorkflow(agentTypes: string[]) {
  ...
  return apiFetch<unknown>("/agents/plan", {
    method: "POST",
    ...
    .then(async (response) => {
      if (!response.ok || !response.body) {
        throw new ApiCallError(
          response.status,
          "SSE_ERROR",
          "Scan progress stream unavailable",
        )
      }
