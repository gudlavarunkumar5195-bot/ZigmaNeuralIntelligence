...
import { apiGetScan, ApiCallError, IntegrationRequired } from "../../services/api";
...
export function ScanProgress() {
  ...
  const load = async () => {
    try {
      ...
      setScan(result.data as ScanRecord);
      setError(null);
    } catch (cause) {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => {
      if (!scan || ACTIVE_STATUSES.has(scan.status)) void load();
    }, 2000);
  ...
  if (error) return <ErrorState title="Unable to load scan execution" message={error.message} onRetry={load} />;
