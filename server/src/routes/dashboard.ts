  ...
  fastify.get("/", { preHandler: [authenticate, requireOrgMember] }, async (request, reply) => {
    ...
    const { rows: websites } = await query(
      `SELECT w.id, w.url, w.domain, w.verified, w.created_at,
              ...
              COALESCE((SELECT ss.score FROM scan_scores ss WHERE ss.scan_id = s.id AND ss.category = 'overall'), NULL) AS overall_score,
              COALESCE((SELECT COUNT(*)::int FROM findings f WHERE f.scan_id = s.id), 0) AS finding_count,
              COALESCE((SELECT COUNT(*)::int FROM findings f WHERE f.scan_id = s.id AND f.severity = 'critical'), 0) AS critical_count
       FROM websites w
       LEFT JOIN LATERAL (
    ...
    const [scores, findings, history, executions] = await Promise.all([
      scanId ? query("SELECT category, score, status, finding_count, critical_count FROM scan_scores WHERE scan_id = $1", [scanId]) : Promise.resolve({ rows: [] }),
      scanId ? query(`SELECT id, category, severity, title, description, recommendation, module_name, created_at FROM findings WHERE scan_id = $1 ORDER BY CASE severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 WHEN 'low' THEN 3 ELSE 4 END, created_at LIMIT 8`, [scanId]) : Promise.resolve({ rows: [] }),
