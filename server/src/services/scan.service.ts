...
async function runScanWithLease(scanId: string): Promise<void> {
  ...
  await query(
    "UPDATE scans SET status = $2, completed_at = NOW() WHERE id = $1 AND org_id = $3 AND execution_owner = $4 AND execution_lease_until > NOW() AND status <> 'cancelled'",
...
async function snapshotMonitoring(
  ...
  const { rows: scoreRows } = await query<{
    category: string
    score: number | null
  }>("SELECT category, score FROM scan_scores WHERE scan_id = $1", [scanId])
...
async function processNextQueuedScan(): Promise<void> {
  ...
  try {
    ...
    const failed = await query<{ org_id: string }>(
      "SELECT org_id FROM scans WHERE id=$1",
      [scanId],
    )
    if (failed.rows[0])
      await import("./monitoring.service.js")
        .then(({ finalizeMonitoringRun }) =>
          finalizeMonitoringRun(scanId, failed.rows[0].org_id, "failed"),
        )
        .catch(() => undefined)
  }
}
